                ...
                <c:choose>
                	<c:when test="${logList.size()!=0}">
                		<c:forEach items="${logList}" var="log">
			                <tr class="body on">
			                    <td class="finish">
			                    	<fmt:formatDate value="${log.log_date}" pattern="yyyy-MM-dd HH:mm:ss" />
			                    </td>
			                    <td class="lead">
			                    	<p style="text-indent: -9999px; display: inline-block;">${log.log_time}</p>
			                    </td>
			                    <td class="name">${log.license_name}</td>
			                    <td class="day">${log.year_date}</td>
			                    <td class="grade">
			                    	<c:forEach items="${log.subjects_current}" var="current">
			                    		<p style="text-indent: -9999px; display: inline-block;">${current}</p>
			                    	</c:forEach>
			                    </td>
			                </tr>
		                </c:forEach>
