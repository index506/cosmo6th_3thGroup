...
<script>
		...
		function go_order(){
			...
			length가 1일때 왜 4가 나오는지?
		    length가 0일때는 어떤 값이 나오는지?
			*/
				
			if(document.getElementsByName("frm").length == 0)
				alert("장바구니에 상품이 없습니다.");
			else if(confirm("주문결제 페이지로 이동하시겠습니까?"))
				window.location.href = "${contextPath}/book/order.do";
			  //window.location.href : 현재페이지를 주소로 이동시킨다.
		}

        function go_cart_update(stsIndex){
        	
       		if(document.getElementsByName('frm').length == 1){
        ...
        function go_cart_delete(cseq){ // [예]를 누르면 서블릿 cartDelete.do 실행
        	if(confirm("삭제하시겠습니까?")){
        		window.location.href = "${contextPath}/book/cartDelete.do?cseq=" + cseq;        		
        	} 
        }
...
<body>
    ...
    <div id="wrap">
		                ...
		                <c:forEach var="cartVO" items="${cartList}" varStatus="status">
		                	<form name="frm" method="post" class="frm">
				                <tr>
				                    ...
				                    <td>
				                        <a href="javascript:go_cart_delete(${cartVO.cseq});" style= "color: black" class="deleteBtn">X</a>
				                    </td>
				                </tr>
			                </form>
		                </c:forEach>
        ...
        <p class="buttonBox">
          <!-- href="javascript:함수();" -->
	        <a href="javascript:go_order();" class="button">결제하기</a>
