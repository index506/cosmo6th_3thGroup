...
//	private static final long serialVersionUID = 1L;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String subject_quiznums = subjectVO.getSubject_quiznums();
		int subject_total = subjectVO.getSubject_total(); // 과목 수
		int subject_time = subjectVO.getSubject_time(); // 시험시간
		
		JSONObject totalObject = new JSONObject();
			JSONObject subjectInfo = new JSONObject(); // 과목 정보를 담는 객체

		subjectInfo.put("subject_names", subject_names);
		subjectInfo.put("subject_quiznums",subject_quiznums);
